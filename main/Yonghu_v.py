...
def yonghu_save(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        if yonghu.objects.filter(yonghuming = req_dict['yonghuming']).count()>0:
            msg['code'] = crud_error_code
            msg['msg'] = "用户名已存在"
            return JsonResponse(msg)
        tablename=request.session.get("tablename")
...
def yonghu_update(request):
    ...
    if request.method in ["POST", "GET"]:
        msg = {"code": normal_code, "msg": mes.normal_code, "data": {}}
        req_dict = request.session.get("req_dict")
        if yonghu.objects.filter(id__gt=req_dict['id'],yonghuming = req_dict['yonghuming']).count()>0:
            msg['code'] = crud_error_code
            msg['msg'] = "用户名已存在"
            return JsonResponse(msg)
...
def yonghu_o_value(request, xColumnName, yColumnName):
    if request.method in ["POST", "GET"]:
        ...
        sql = "SELECT {0}, ROUND(sum({1}),2) AS total FROM yonghu {2} GROUP BY {0} LIMIT 10".format(xColumnName, yColumnName, where)
        func_name = sys._getframe().f_code.co_name
        table_name = func_name.split('_')[0]
